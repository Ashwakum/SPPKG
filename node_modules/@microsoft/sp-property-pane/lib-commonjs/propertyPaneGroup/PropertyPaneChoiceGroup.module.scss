...
.field {
  ...
  &.fieldIsChecked {
    &::before {
      // the circle
      border: 2px solid;
      color: $radioButton-border-selected-color;
...
.fieldIsIcon {
  ...
  .labelWrapperTextOverflow {
    // replace 'line-height' value in .labelWrapper with this line
    // when this KS (7454c194-0ee3-40ad-8de8-b37e6b1ab9c1) is graduated
    $lineHeight: 16px;
    // we add 3 lines to support scenarios when the user enables Text Spacing
    height: $lineHeight * 3;
  }
  ...
  @include ms-RTL {
    &.RTLEnabled {
      ...
      &::before {
        // right needs to be auto in RTL mode
        right: auto !important; // To reset the value of 'right' to its default value, so that 'left' works
      }
  ...
  @include ms-LTR {
    &.RTLEnabled {
      &::before {
        // left needs to be auto in LTR mode
