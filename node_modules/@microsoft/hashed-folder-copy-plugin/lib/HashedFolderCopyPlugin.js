...
const ParserHelpers = node_core_library_1.Import.lazy('webpack/lib/ParserHelpers', require);
// tslint:disable-next-line:variable-name
const ConstDependency = node_core_library_1.Import.lazy('webpack/lib/dependencies/ConstDependency', require);
...
class HashedFolderCopyPlugin {
    apply(compiler) {
        compiler.hooks.normalModuleFactory.tap(PLUGIN_NAME, (normalModuleFactory) => {
            const normalResolver = normalModuleFactory.getResolver('normal', { useSyncFileSystemCalls: true });
            const handler = (baseParser) => {
                ...
                parser.hooks.call.for(EXPRESSION_NAME).tap(PLUGIN_NAME, (expression) => {
                    ...
                    const dependency = new ConstDependency(`/* ${EXPRESSION_NAME} */ ${dependencyText}`, expression.range, false);
    ...
    _collectAssets(options) {
        ...
        for (const source of requireFolderOptions.sources) {
            ...
            if (globsBase.startsWith('.')) {
                ...
                let packagePath;
                try {
                    const resolveResult = resolver.resolveSync({}, module.context, `${packageName}/package.json`);
